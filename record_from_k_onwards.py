...
CLASSES = [chr(i) for i in range(ord('k'), ord('z')+1)] + ['space', 'enter', 'noise']
...
def record_realtime(class_name, sample_idx):
    print(f"\n🎬 Recording sample {sample_idx + 1} for '{class_name}'")
    if class_name == "noise":
        print("👉 Make any background noise in 2 seconds...")
    elif class_name == "space":
        print("👉 Press the SPACE key in 2 seconds...")
    elif class_name == "enter":
        print("👉 Press the ENTER key in 2 seconds...")
    else:
    ...
    if len(audio) == 0 or np.max(np.abs(audio)) < 0.01:
        print(f"⚠️ Too quiet or empty audio for {class_name}, sample {sample_idx}")
        return None
...
def main():
    print("🔁 Resuming dataset collection (k to z + specials)")
    ...
    for class_name in CLASSES:
        ...
        print(f"🗂️ {existing_count} existing samples. Need {remaining} more.")
        ...
        print(f"✅ Completed {TARGET_SAMPLES_PER_CLASS} samples for '{class_name}'")

    print("\n🎉 All remaining data collection done!")
